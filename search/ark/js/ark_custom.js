...
var arkId = "#autoList";						// 자동완성 전체 <div> 의 id을 설정한다
...
var target = "isky";								// ARK 웹서버 설정파일의 목록에 있는 추천어 서비스 대상을 지정한다.
...
function drawArk() {
	...
	// 자동완성 닫기 버튼
	str += "<a id=\"closeBtn\" href=\"javascript:hideArk();\" class=\"closeBtn\" ><img src=\"../img/btn/btn_close02.gif\" alt=\"자동완성접기\" /></a>";	
	
	// 자동완성 결과 생성
	str += "	<ul id=\"" + contentListId + "\">";
//	str += "		<li>";
//	str += "			<a href=\"#\" id=\"" + contentListId + "\"></a>";
//	str += "		</li>";
			...
			$.each(data.result, function(i, result) {
				...
				if (totalCount > 0) {
					...
					$.each(result.items, function(num,item){
						...
						str += "<li><a href='#'><strong>" + keyword + "</strong></a></li>";
...
//				}
				...
				$resultElement.find("ARKList").each(function(idx) {
					$(this).find("ARK").each(function(idx) {
						...
						hkeyword = hkeyword.replace("<font style='font-size:13px'><font style='color:#5aacff'>","<strong>");
...
//						str += "<li><a href='#'><strong>" + keyword + "</strong></a></li>";
						str += "<li onclick=\"onClickKeyword(" + idx + ");\" onmouseover=\"onMouseOverKeyword(" + idx + ");\"";
						str += " onmouseout=\"onMouseOutKeyword(" + idx + ");\">" + "<a id=\"f" + idx +"\" href=\"javascript:arkSearch('" + keyword + "');\">" + hkeyword + "</a>";
						str += " <span style=display:none;>" + keyword + "</span>";
						str += "</li>";
					});
				});
...
function moveFocusEvent(event) {
	...
	if (event.keyCode == 38) {
		...
		if ((totalFwCount) > (cursorPos + 1)) { // totalFwCount 값만 받음. totalRwCount 값은 안쓰는 관계로.
			onMouseOutKeyword(cursorPos);
			cursorPos = cursorPos + 1;
			onMouseOverKeyword(cursorPos);
			$(queryId).val($("#f" + cursorPos).text());
...
function onMouseOverKeyword(cursorNum) {
	clearCursorPos();
	cursorPos = cursorNum;
	$("#f" + cursorNum).css({"backgroundColor" : "#eeeeee"});
